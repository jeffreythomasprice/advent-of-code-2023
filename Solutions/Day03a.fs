...
let doIt (input: string) : int =
    ...
    for shape in shapes do
        ...
        | Symbol _ -> symbols.Add shape
        | Number _ -> numbers.Add shape
        | _ -> ()

    let isAdjacentToSymbol x =
        (symbols |> Seq.tryFind (fun s -> s.IsAdjacent x)).IsSome

    let partNumbers =
        numbers |> Seq.filter (fun x -> isAdjacentToSymbol x) |> Seq.toArray

    let otherNumbers =
        numbers
        |> Seq.filter (fun x -> not (Array.contains x partNumbers))
        |> Seq.toArray
    ...
    printfn "PART NUMBERS"

    for x in partNumbers do
        printfn "%A" x

    printfn ""

    printfn "OTHER NUMBERS"

    for x in otherNumbers do
        printfn "%A" x
