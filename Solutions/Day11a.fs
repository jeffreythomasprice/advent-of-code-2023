    ...
    let updatedStarLocations =
        ...
        |> Seq.map (fun (x, y) ->
            let emptyColumnsBeforeHere =
                emptyColumns |> Seq.filter (fun column -> x > column) |> Seq.length

            let emptyRowsBeforeHere = emptyRows |> Seq.filter (fun row -> y > row) |> Seq.length
            (x + emptyColumnsBeforeHere, y + emptyRowsBeforeHere))
